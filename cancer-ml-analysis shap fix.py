...
def analyze_outcome(data, outcome_column):
    ...
    try:
        ...
        feature_importance_df = pd.DataFrame({
            ...
            'SHAP Importance': np.abs(shap_values_instance)
        ...
        # Filter SHAP values & feature names for top 5 only
        selected_features = top_5_features['Feature'].tolist()
        shap_values_filtered = shap_values_instance[top_5_features.index]

        # Create SHAP Explanation object (Top 5 Features Only)
        shap_exp = shap.Explanation(
            values=shap_values_filtered,
            base_values=base_value,
            data=X_test_sample.iloc[instance_idx][selected_features].values,  # Filtered features
            feature_names=selected_features
        )

        print(f"\nFinal SHAP Base Value: {base_value}")

        # 🚀 **Plot waterfall for Top 5 Features**
        plt.figure(figsize=(10, 6))
        shap.waterfall_plot(shap_exp)
        plt.title(f'SHAP Waterfall Plot (Top 5 Features) - {outcome_column}')
        plt.tight_layout()
        plt.show()
    ...
    sns.barplot(x='SHAP Importance', y='Feature', data=top_5_features, palette="Blues_r")
    plt.title(f'Top 5 Features - {outcome_column}')
